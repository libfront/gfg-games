    ...
    window.RuntimeInterface = class RuntimeInterface {
        ...
        async _Init(opts) {
            ...
            if (opts.workerScripts)
                for (const [url,
                        blob
                    ] of Object.entries(opts.workerScripts)) this._workerScriptBlobURLs[url] = URL.createObjectURL(blob);
    ...
    const HANDLER_CLASS = class RuntimeDOMHandler extends self.DOMHandler {
        ...
        _TrySetTargetOrientation() {
            ...
            if (screen["orientation"] && screen["orientation"]["lock"]) screen["orientation"]["lock"](orientation).catch(err => console.warn("[Construct 3] Failed to lock orientation: ", err));
                ...
                else if (screen["mozLockOrientation"]) result =
        ...
        _OnMouseEventAsPointer(name, e) {
            ...
            if (name ===
                "pointerdown" && lastButtons !== 0) name = "pointermove";
        ...
        _OnPointerEvent(name, e) {
            ...
            if (e.pointerType === "mouse") {
                ...
                else if (name === "pointerup") mouseEventName = "pointerup";
...
'use strict'; {
    ...
    self.JobSchedulerDOM = class JobSchedulerDOM {
        constructor(runtimeInterface) {
            ...
            if (runtimeInterface.GetExportType() === "preview") this._baseUrl += "c3/workers/";
...
'use strict'; {
    ...
    const HANDLER_CLASS = class BrowserDOMHandler extends self.DOMHandler {
        ...
        _OnNavigate(e) {
            ...
            if (type === "back")
            ...
            else if (type === "forward") window.forward();
            else if (type === "home") window.home();
                ...
                if (exportType === "windows-uwp" && typeof Windows !==
                    "undefined") Windows["System"]["Launcher"]["launchUriAsync"](new Windows["Foundation"]["Uri"](url));
                else if (self["cordova"] && self["cordova"]["InAppBrowser"]) self["cordova"]["InAppBrowser"]["open"](url, "_system");
                else if (exportType === "preview") window.open(url, "_blank");
                ...
                const tag = e["tag"];
                const exportType = e["exportType"];
                if (exportType === "windows-uwp" && typeof Windows !== "undefined") Windows["System"]["Launcher"]["launchUriAsync"](new Windows["Foundation"]["Uri"](url));
                else if (self["cordova"] && self["cordova"]["InAppBrowser"]) self["cordova"]["InAppBrowser"]["open"](url, "_system");
...
'use strict'; {
    ...
    self.AudioDOMHandler = class AudioDOMHandler extends self.DOMHandler {
        ...
        async _AddEffect(e) {
            ...
            else if (type === "delay") effect = new self.C3AudioDelayFX(this,
                ...params);
            ...
            else if (type === "phaser") effect = new self.C3AudioPhaserFX(this,
                ...params);
...
'use strict'; {
    self.C3WebAudioBuffer = class C3WebAudioBuffer extends self.C3AudioBuffer {
        ...
        async _Fetch() {
            ...
            if (iRuntime.GetExportType() === "cordova" && iRuntime.IsRelativeURL(this._url) && location.protocol === "file:") this._audioData = await iRuntime.CordovaFetchLocalFileAsArrayBuffer(this._url);
            else {
...
'use strict'; {
    self.C3WebAudioInstance = class C3WebAudioInstance extends self.C3AudioInstance {
        ...
        GetPlaybackTime(applyPlaybackRate) {
            ...
            else ret = this.GetCurrentTime() - this._startTime;
            if (applyPlaybackRate) ret *= this._playbackRate;
            if (!this._isLooping) ret = Math.min(ret, this.GetDuration());
            return ret
        }
...
'use strict'; {
    ...
    const HANDLER_CLASS = class MobileAdvertHandler extends self.DOMHandler {
        ...
        _GetPlugin() {
            ...
            else if (USE_EMULATOR) return pluginEmulator
        }
