'use strict'; {
    ...
    async function InitRuntime(data) {
        ...
        const engineScripts = data["engineScripts"].map(url => (new URL(url, baseUrl)).toString());
        try {
            await LoadScripts([...workerDependencyScripts,
                ...engineScripts
            ])
        } catch (err) {
        ...
        if (projectScripts && projectScripts.length > 0) {
            ...
            try {
                ...
                ReportProjectScriptError(projectScripts, scriptsStatus, data["messagePort"]);
                return
            }
        }
        data["runOnStartupFunctions"] =
            runOnStartupFunctions;
        if (data["exportType"] === "preview" && typeof self.C3.ScriptsInEvents !== "object") {
    ...
    async function ReportProjectScriptError(projectScripts,
        scriptsStatus, messagePort) {
        let msg;
        for (const [originalUrl, src] of projectScripts) {
            ...
            try {
                ...
                if (originalUrl === "scriptsInEvents.js") msg = "Failed to load JavaScript code used in events. Check all your JavaScript code has valid syntax.";
                else msg = `Failed to load project script '${originalUrl}'. Check all your JavaScript code has valid syntax.`;
